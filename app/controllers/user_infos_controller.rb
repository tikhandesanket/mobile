class UserInfosController < ApplicationController
  ...
  def first_step
    user_info = params[:info_type]

    case user_info
      when 'personal_info'
        session[:name] = params[:name]
        session[:e_mail] = params[:email]
      when 'company_info'
        session[:company_name] = params[:company_name]
        session[:position] = params[:position]
      when 'education_info'
        session[:education] = params[:education]
        session[:mark] = params[:mark]
    end
    @user_info = UserInfo.new(:name=> session[:name],:e_mail=>session[:email])
    @user_info.save
  ...
  private
    ...
    def user_info_params
      params.require(:user_info).permit(:name, :e_mail, :company_name, :position, :education, :mark)
    end
end
